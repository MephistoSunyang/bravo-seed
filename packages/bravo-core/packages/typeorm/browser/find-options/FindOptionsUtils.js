...
var FindOptionsUtils = /** @class */ (function () {
    ...
    FindOptionsUtils.applyRelationsRecursively = function (qb, allRelations, alias, metadata, prefix) {
        ...
        matchedBaseRelations.forEach(function (relation) {
            ...
            if (relMetadata) {
                if (relMetadata.entityMetadata.deleteColumn) {
                    qb.leftJoinAndSelect(selection, relationAlias, relationAlias + "." + relMetadata.entityMetadata.deleteColumn.databaseName + " = 0");
                }
                else if (relMetadata.entityMetadata.deleteDateColumn) {
                    qb.leftJoinAndSelect(selection, relationAlias, relationAlias + "." + relMetadata.entityMetadata.deleteDateColumn.databaseName + " IS NULL");
                }
                else {
    ...
    FindOptionsUtils.joinEagerRelations = function (qb, alias, metadata) {
        ...
        metadata.eagerRelations.forEach(function (relation) {
            ...
            if (relation.entityMetadata.deleteColumn) {
                qb.leftJoinAndSelect(alias + "." + relation.propertyPath, relationAlias, relationAlias + "." + relation.entityMetadata.deleteColumn.databaseName + " = 0");
            }
            else if (relation.entityMetadata.deleteDateColumn) {
                qb.leftJoinAndSelect(alias + "." + relation.propertyPath, relationAlias, relationAlias + "." + relation.entityMetadata.deleteDateColumn.databaseName + " IS NULL");
            }
            else {
