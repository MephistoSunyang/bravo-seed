...
var QueryBuilder = /** @class */ (function () {
    ...
    QueryBuilder.prototype.createWhereExpression = function () {
        ...
        if (this.expressionMap.mainAlias.hasMetadata) {
            ...
            if (this.expressionMap.queryType === "select" && !this.expressionMap.withDeleted && (metadata.deleteDateColumn || metadata.deleteColumn)) {
                if (metadata.deleteColumn) {
                    var column = this.expressionMap.aliasNamePrefixingEnabled
                        ? this.expressionMap.mainAlias.name + "." + metadata.deleteColumn.propertyName
                        : metadata.deleteColumn.propertyName;
                    var condition = this.replacePropertyNames(column) + " = 0";
